...
public class Partida {
    ...
    public int pruebaCasilla(int f, int c) {
        int tamBarco, filaInicial, colInicial;
        char orientacion;
        
    	int resulDisparo = mar[f][c];
    	
    	if(resulDisparo==AGUA||resulDisparo==TOCADO||resulDisparo==HUNDIDO)
    		return mar[f][c];
    			...
    			if(barco.getTocadas() == barco.getTamanyo()){
    				...
    				if(orientacion == 'H'){
    					for(int i=0 ; i<tamBarco ; i++){
    						mar[filaInicial][colInicial+i] = HUNDIDO;
    					}
    				}
    				else{
    					for(int i=0 ; i<tamBarco ; i++){
